...
public partial class CG_ConsultaSaldos : System.Web.UI.Page
{
    ...
    protected void rCboCuentas_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
    {
        LlenaGrid();
    }
    protected void rCboCuentas_ItemChecked(object sender, RadComboBoxItemEventArgs e)
    {
        LlenaGrid();
    }
    protected void rCboCuentas_CheckAllCheck(object sender, RadComboBoxCheckAllCheckEventArgs e)
    {
        LlenaGrid();
    }
    ...
    {
        FNGrales.bTitleDesc(Page, "Consulta de Saldos Cuentas Contables", "PnlMPFormTituloApartado");
    }
    private void InicioLlenaControles()
    {
        ...
        FnCtlsFillIn.RadComboBox_CodificacionCuentas(Pag_sConexionLog, Pag_sCompania, ref rCboCuentas, true, false);
    ...
    private void LlenaGrid()
    {
        ...
        if (rCboCuentas.SelectedIndex != -1) { 
        ProcBD.AgregarParametrosProcedimiento("@ctaContCve", DbType.String, 20, ParameterDirection.Input, rCboCuentas.SelectedValue);
        }
    ...
    private void EjecutaAccion()
    {
        ...
        if (msgValidacion == "")
        {
            ...
            string sCtaContCve = dataItem["ctaContCve"].Text;

            //SALDOS MENSUAL
            if (hdfBtnAccion.Value == Convert.ToUInt16(MGMValores.Constantes.btnAccion.SaldoMensual).ToString())
            {
                string sScript = "<script>openPopUpSaldosMes('" + rCboAnioPeriodo.SelectedValue + "',' " + rCboNumPeriodo.SelectedValue + "',' " + sCtaContCve + "')</script>";
            ...
            if (hdfBtnAccion.Value == Convert.ToUInt16(MGMValores.Constantes.btnAccion.SaldoMov).ToString())
            {
                string sScript = "<script>openPopUpSaldosMov('" + rCboAnioPeriodo.SelectedValue + "',' " + rCboNumPeriodo.SelectedValue + "',' " + sCtaContCve + "')</script>";
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "Key", sScript, false);
            }
    ...
    private void ShowCheckedItems(RadComboBox comboBox)
    {
        ...
        if (collection.Count != 0)
        {
            ...
            foreach (var item in collection)
            {
                ...
                ProcBD.AgregarParametrosProcedimiento("@ciaCve", DbType.String, 10, ParameterDirection.Input, Pag_sCompania);
                ProcBD.AgregarParametrosProcedimiento("@maUsuCve", DbType.String, 20, ParameterDirection.Input, maUser);
                ProcBD.AgregarParametrosProcedimiento("@FilCve", DbType.String, 20, ParameterDirection.Input, "CONSULTASALDOCG");
                ProcBD.AgregarParametrosProcedimiento("@Cve", DbType.String, 20, ParameterDirection.Input, Cve);
