...
public partial class CC_AdmonCobrosRegistro : System.Web.UI.Page
{
    ...
    private void llenaCombos()
    {
        ...
        FnCtlsFillIn.RabComboBox_Clientes(Pag_sConexionLog, Pag_sCompania, ref rCboBeneficiario, true, false);
    }
    ...
    private void LlenarUi(string docRegId)
    {
        ...
        try
        {
            ...
            if (FnValAdoNet.bDSIsFill(ds))
            {
                ...
                RdDateFecha.SelectedDate = Convert.ToDateTime(ds.Tables[0].Rows[0]["cobCCFec"].ToString());
                rCboConcepto.SelectedValue = ds.Tables[0].Rows[0]["cptoId"].ToString();
                //rCboBeneficiario.SelectedValue = ds.Tables[0].Rows[0]["cliCve"].ToString();
                rTxtDescripcion.Text = ds.Tables[0].Rows[0]["cobCCDes"].ToString();
                rTxtImporte.Text = ds.Tables[0].Rows[0]["cobCCImp"].ToString();
                rTxtImporte.Value = Convert.ToDouble(ds.Tables[0].Rows[0]["cobCCImp"].ToString());
                rTxtFolio.Text = ds.Tables[0].Rows[0]["copCCPFolio"].ToString();
                ...
                {
                    Session["dsTransDetCCSession"] = ds;
                }
    ...
    private void TituloPagina()
    {
        ...
        {
            FNGrales.bTitleDesc(Page, "Nuevo Registro - Administración de Cobros", "PnlMPFormTituloApartado");
        }
        else if (hdfBtnAccion.Value == "2")
        {
            FNGrales.bTitleDesc(Page, "Editar Registro - Administración de Cobros", "PnlMPFormTituloApartado");
        }
    ...
    private void EjecutaAccionDetalle_Trans()
    {
        ...
        if (hdfBtnAccionDet.Value == Convert.ToUInt16(MGMValores.Constantes.btnAccion.Modificar).ToString())
        {
            ...
            Session["dataItemSessionCC"] = stransDetId;
        ...
        try
        {
            ...
            if (FnValAdoNet.bDSIsFill(ds))
            {
                ...
                if (sEjecEstatus == "1")
                {
                    ...
                    if (rBtnRegTransaccion.Checked == true)
                    {
                        GuardaTransaccionDetalle(sEjectransId, rTxtFolio.Text);
    ...
    private void GuardaTransaccionDetalle(string transId, string Folio)
    {
        int ValEliminar = 0;
        DataSet dsTransDet = new DataSet();
        dsTransDet = (DataSet)Session["dsTransDetCCSession"];
        ...
        if (FnValAdoNet.bDSIsFill(ds))
        {
            ...
            {
                PagosXp(Folio, transId);
            }
        }

    }

    public void PagosXp(string Folio, string prmTranId)
    {
        try
        {
            string fchPrmSp;
            ...
            if (hdfBtnAccion.Value == "2")
            {
                ProcBD.AgregarParametrosProcedimiento("@cobCCId", DbType.Int64, 0, ParameterDirection.Input, Convert.ToInt64(Pag_sIdDocReg));
            }

            ProcBD.AgregarParametrosProcedimiento("@cptoId", DbType.Int64, 0, ParameterDirection.Input, rCboConcepto.SelectedValue);
            ProcBD.AgregarParametrosProcedimiento("@cobCCFolio", DbType.String, 10, ParameterDirection.Input, Folio);
            ProcBD.AgregarParametrosProcedimiento("@ctaDepCve", DbType.String, 10, ParameterDirection.Input, rCboCuenta.SelectedValue);
            ProcBD.AgregarParametrosProcedimiento("@cobCCFec", DbType.String, 100, ParameterDirection.Input, fchPrmSp);
            //ProcBD.AgregarParametrosProcedimiento("@cliCve", DbType.String, 10, ParameterDirection.Input, rCboBeneficiario.SelectedValue);
            ProcBD.AgregarParametrosProcedimiento("@cobCCDes", DbType.String, 50, ParameterDirection.Input, rTxtDescripcion.Text);
            //ProcBD.AgregarParametrosProcedimiento("@cobCCNum", DbType.String, 10, ParameterDirection.Input, "");
            ProcBD.AgregarParametrosProcedimiento("@cobCCImp", DbType.Decimal, 19, ParameterDirection.Input, Convert.ToDecimal(rTxtImporte.Text));
            ProcBD.AgregarParametrosProcedimiento("@cobCCSitEnt", DbType.String, 10, ParameterDirection.Input, "");
            ProcBD.AgregarParametrosProcedimiento("@cobCCSit", DbType.String, 10, ParameterDirection.Input, "R");
            ...
            if (FnValAdoNet.bDSIsFill(ds))
            {
                ...
                if (sEjecEstatus == "1")
                {
                    ...
                    if (hdfBtnAccion.Value == "1")
                    {
                        Session["Valor_DocCve"] = ds.Tables[0].Rows[0]["cobCCId"].ToString();
                        Session["Valor_btn"] = "2";
    ...
    private void Movimientos_DatosDetalle()
    {
        ...
        //Session["dsTransDetSession"] = dsMovDet;
    ...
    private void GuardaMovimientos_Captura(string transId)
    {
        ...
        if (FnValAdoNet.bDSIsFill(ds))
        {
            ...
            if (sEjecEstatus.ToString() == "1")
            {
                PagosXp(rTxtFolio.Text, transId);
            }
    ...
    private void EliminaAsiContDeOpeyAdmon(string transId)
    {
        try
        {
            ...
            ProcBD.AgregarParametrosProcedimiento("@Modulo", DbType.String, 5, ParameterDirection.Input, "CC");
