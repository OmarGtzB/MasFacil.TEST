...
public partial class XP_AdmonPagosRegistroTrans : System.Web.UI.Page
{
    ...
    protected void rBtnBuscar_Click(object sender, ImageButtonClickEventArgs e)
    {
        ...
        if (divForm.Visible)
        {
            //GuardarValores no de Busqueda para despues ser recuperados
    ...
    {      
        hdfBtnAccionDet.Value = Session[("hdfBtnAccionDetXP")].ToString();
        LlenaCboMoneda();
        VisualizaModal();
        loadGridPP();

        this.RadWindowManagerPage.Width = 900;
        ...
        //setValueDataList(getColumn("cptoConfId_Ref10_CodApli"), Convert.ToString(Session["refPrincXP"]), false);
        //setValueDataList(getColumn("cptoConfId_Fec_Mov"), Convert.ToString(Session["fchMovXP"]), true);
    ...
    private void setValueDataList(string columnGet, string valueSet, bool enabled)
    {
        ...
        foreach (DataListItem dlConf in DataList1.Items)
        {
            ...
            if (columnGet == keysDataList_colum)
            {
                ...
                if (objRadTextBox.Visible == true)
                {
                    ...
                    objRadTextBox.Enabled = enabled;
                }

                if (objRadNumericTextBox.Visible == true)
                {
                    objRadNumericTextBox.Text = valueSet;
                    objRadNumericTextBox.Enabled = enabled;
                }
                ...
                if (objRadDatePicker.Visible == true)
                {
                    ...
                    objRadDatePicker.Enabled = enabled;
                    /*
                    sValor = "";
                    string Val_Fec = objRadDatePicker.SelectedDate.ToString();
                    if (Val_Fec != "")
                ...
                if (objRadComboBox.Visible == true)
                {
                    ...
                    objRadComboBox.Enabled = enabled;
                }
    ...
    public void recorreDatalist(DataSet dsConfConcepto)
    {
        ...
        foreach (DataRow drConfigCpto in dsConfConcepto.Tables[0].Rows)
        {
            ...
            //GRET
            String cptoConfDes = drConfigCpto["cptoConfDes"].ToString();
            //
            ...
            foreach (DataListItem DtalistFila in DataList1.Items)
            {
                ...
                if (controlador.ToString() == Row)
                {
                    ...
                    //GRET, se agrega codigo para que aparesca en automatico la etiqueta @@CHEQUE@@ y el campo quede deshabilitado para usuario cada que se de de alta un nuevo pago
                    if (hdfBtnAccionDet.Value == Convert.ToUInt16(MGMValores.Constantes.btnAccion.Nuevo).ToString())
                    {
                        if (cptoConfDes == "No. Cheque" | cptoConfDes == "Cheque" | cptoConfDes == "No cheque" | cptoConfDes == "Folio Cheque")
                        {
                            objRadTextBox.Text = "@@CHEQUE@@";
    ...
    {
        string Pag_sTransDetId = Session["dataItemSessionXP"].ToString();
    ...
    private void loadGridPP()
    {

        string prmProvCve = getValueDataList(getColumn("cptoConfId_Ref10_CodApli"));
        ...
        try
        {
            ...
            if (prmProvCve != "")
            {
                ProcBD.AgregarParametrosProcedimiento("@provCve", DbType.String, 10, ParameterDirection.Input, prmProvCve);
            }
            else
            {
                ProcBD.AgregarParametrosProcedimiento("@provCve", DbType.String, 10, ParameterDirection.Input, "");
            }
            ...
            if (prmImpAbo != "")
            {
                ProcBD.AgregarParametrosProcedimiento("@partPenPagImpAbo", DbType.Decimal, 15, ParameterDirection.Input, Convert.ToDecimal(prmImpAbo));
            }


            if (prmFecMov != "")
            {
                ProcBD.AgregarParametrosProcedimiento("@partPenPagFecMov", DbType.String, 100, ParameterDirection.Input, prmFecMov.Replace('/', '-') + " 00:00:00");
            }

            if (prmFecVenc != "")
            {
                ProcBD.AgregarParametrosProcedimiento("@partPenPagFecVenc", DbType.String, 100, ParameterDirection.Input, prmFecVenc.Replace('/', '-') + " 00:00:00");
            }
            ...
            else
            {
                for (int i = 0; i < rGdvPagosPend.Items.Count; i++)
                {
                    ...
                    if (rGdvPagosPend.Items[i].Selected == true)
                    {
                        ...
                        if (getColumn("cptoConfId_Ref10_Princ").Trim() != "")
                        {
                            dr[getColumn("cptoConfId_Ref10_Princ")] = rGdvPagosPend.Items[i].Cells[7].Text;
                        }
                        */
                        ...
                        {
                            dr[getColumn("cptoConfId_Fec_Mov")] = Convert.ToDateTime(Session["fchMovXP"]);
                        }

                        if (getColumn("cptoConfId_Fec_Venc").Trim() != "")
                        {
                            dr[getColumn("cptoConfId_Fec_Venc")] = Convert.ToDateTime(Session["fchVencXP"]); //rGdvPagosPend.Items[i].Cells[4].Text;
                        }
