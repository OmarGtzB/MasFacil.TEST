...
public partial class Menu_MGMApartadosConcepto : System.Web.UI.Page
{
    #region VARIABLES
    ...
    protected void RadAjaxManager1_AjaxRequest(object sender, AjaxRequestEventArgs e)
    {
        if (Pag_sidM == "39")
        {
            if (Session["folio_Selection"].ToString() != "")
            {
                string sCve = Session["folio_Selection"].ToString();
                ReDireccionaPage(sCve);
            }
        }
    ...
    private void PermisoBotones()
    {
        ...
        dtrBtnCopiar.Visible = rBtnCopiar.Visible;
        if (dtrBtnCopiar.Visible == false)
        {
            TBwidth = TBwidth - 55;
        }
    ...
    private void ControlesAccionEjecucion(bool Result)
    {
        if (Result == true)
        {
            ...
            if (hdfBtnAccion.Value == Convert.ToUInt16(MGMValores.Constantes.btnAccion.Copiar).ToString())
            {
                ...
                var dataItem = rGdvRegistros.SelectedItems[0] as GridDataItem;
                string Cve = dataItem.GetDataKeyValue("Cve").ToString();

                RadWindowCptoCopia.NavigateUrl = "../DC/ApartadosConceptoCopia.aspx?Cve=" + Cve;
                string script = "function f(){$find(\"" + RadWindowCptoCopia.ClientID + "\").show(); Sys.Application.remove_load(f);}Sys.Application.add_load(f);";
                ScriptManager.RegisterStartupScript(Page, Page.GetType(), "key", script, true);
    ...
    private void RadWinMenu()
    {
        ...
        if (FnValAdoNet.bDSIsFill(ds))
        {
            ...
            foreach (DataRow Drw in DrwEjecFun)
            {
                ...
                string maMenuId = Convert.ToString(Drw["maMenuId"]).ToString().Trim();
                ...
                if (maMenuId == "40")
                {
                    rwin.OnClientClose = "closeRadWindow";
                }
    ...
    private void ValorMensajeOpcApartado()
    {
        string sResult = "", sMSGTip = "";
        FNDatos.dsMAMensajes(Pag_sConexionLog, "VAL1029", ref sMSGTip, ref sResult);
        hdfMsg.Value = sResult;
    }
    ...
    public string ReDireccionaPage(string valCpto)
    {
        string tipoCpto = "";
        string sResult = "", sMSGTip = "";
        FNDatos.dsMAMensajes(Pag_sConexionLog, "VAL1029", ref sMSGTip, ref sResult);
        ...
        ProcBD.AgregarParametrosProcedimiento("@ciaCve", DbType.String, (10), ParameterDirection.Input, Pag_sCompania);
        ProcBD.AgregarParametrosProcedimiento("@CptoId", DbType.Int64, 0, ParameterDirection.Input, Convert.ToInt32(valCpto));
        ...
        if (tipoCpto.Substring(0,1) != "I") {
            try
            {
                foreach (DataListItem dli in DataListMenu.Items)
                {
                    var RadButton = dli.FindControl("RadButton1") as RadButton;
                    if (RadButton.Value == "149")
                    {
                        RadButton.OnClientClicked = "";
                        RadButton.OnClientClicked = "FNDocAlert";
                        RadButton.NavigateUrl = "";
                    }
                }
            }
            catch (Exception ex)
