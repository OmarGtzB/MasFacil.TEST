...
public partial class DC_MttoCptoMapeoModulo : System.Web.UI.Page
{
    #region VARIABLES
        ...
        try
        {
            ...
            foreach (GridDataItem i in rGdv_MapeoModulo.SelectedItems)
            {
                ...
                if (dataItem != null)
                {
                    ...
                    string SecuenciaCve = dataItem.GetDataKeyValue("movMapCCSecApli").ToString();
                    try
                    {
                        DataSet ds = new DataSet();
                        MGM.ProcedimientoAlmacenado.Procedimiento ProcBD = new MGM.ProcedimientoAlmacenado.Procedimiento();
    ...
    public void CargaDatosEdita()
    {
        ...
        ProcBD.AgregarParametrosProcedimiento("@ciaCve", DbType.String, (10), ParameterDirection.Input, Pag_sCompania);
        ProcBD.AgregarParametrosProcedimiento("@CptoId", DbType.Int64, 0, ParameterDirection.Input, PagLoc_Cpto);
        ProcBD.AgregarParametrosProcedimiento("@movMapCCSecApli", DbType.Int64, 0, ParameterDirection.Input, dataItem["movMapCCSecApli"].Text);
        ds = oWS.ObtenerDatasetDesdeProcedimiento(ProcBD.ObtenerXmlProcedimiento(), Pag_sConexionLog);


        rTxtSecuencia.Text = ds.Tables[1].Rows[0]["movMapCCSecApli"].ToString();

        if (ds.Tables[1].Rows[0]["movMapCCCoA"].ToString() == "1")
        {
            MovimientoAbono.Checked = false;
            MovimientoCargo.Checked = true;

        }
        else if (ds.Tables[1].Rows[0]["movMapCCCoA"].ToString() == "2")
        {
            MovimientoCargo.Checked = false;
            MovimientoAbono.Checked = true;
        }

        if (ds.Tables[1].Rows[0]["movMapCCTipApli"].ToString() == "1")
        {
        ...
        else if (ds.Tables[1].Rows[0]["movMapCCTipApli"].ToString() == "2")
        {
            TipoCtaDeposito.Checked = false;
            TipoCliente.Checked = true;
        }
        rCboSecuenciaCodigo.SelectedValue = ds.Tables[1].Rows[0]["cptoConfId_Ref10_CodApli"].ToString();
        rCboReferenciasAplicacion.SelectedValue = ds.Tables[1].Rows[0]["cptoConfId_Ref10_Aplic"].ToString();
        rCboReferenciasPrincipal.SelectedValue = ds.Tables[1].Rows[0]["cptoConfId_Ref10_Princ"].ToString();
        rCboReferencia3.SelectedValue = ds.Tables[1].Rows[0]["cptoConfId_Ref10_03"].ToString();
        rCboReferencia4.SelectedValue = ds.Tables[1].Rows[0]["cptoConfId_Ref10_04"].ToString();
        rCboMovimiento.SelectedValue = ds.Tables[1].Rows[0]["cptoConfId_Fec_Mov"].ToString();
        rCboVencimiento.SelectedValue = ds.Tables[1].Rows[0]["cptoConfId_Fec_Venc"].ToString();
        rCboMoneda.SelectedValue = ds.Tables[1].Rows[0]["monCve"].ToString();
        rCboImporte.SelectedValue = ds.Tables[1].Rows[0]["cptoConfId_Imp_Imp"].ToString();
        rCboTipCambio.SelectedValue = ds.Tables[1].Rows[0]["cptoConfId_Fac_TipCam"].ToString();
        rCboDescripcion.SelectedValue = ds.Tables[1].Rows[0]["cptoConfId_Ref40_Des"].ToString();
    ...
    public void ReDireccionaPage()
    {
        ...
        if (tipoCpto == "CG")
        {
            Response.Redirect("CG.aspx?idM=" + Pag_sidM);
        }
        if (tipoCpto == "XP")
        {
            Response.Redirect("MttoCptoMapeoModuloXP.aspx?idM="+ Pag_sidM);
        }
        tipoCpto = tipoCpto.Substring(0, 1);

        if (tipoCpto == "I")
        {
            Response.Redirect("MttoCptoMapeoModuloIn.aspx?idM=" + Pag_sidM);
        }
