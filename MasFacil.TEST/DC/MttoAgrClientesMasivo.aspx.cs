...
public partial class DC_MttoAgrClientesMasivo : System.Web.UI.Page
{
    #region VARIABLES
    ...
    protected void rBtnBuscar_Click(object sender, ImageButtonClickEventArgs e)
    {
        ...
        if (rCboAgrupaciones.SelectedValue != "" && rCboAgrupacionesDatos.SelectedValue != "")
        {
            ...
            dsAlmArt = (DataSet)Session["dsRelAgrClieSession"];
            ...
            if (RdioBtnTodos.Checked == true)
            {
                ...
                {
                    DataListClie.DataSource = dsAlmArt;
                    DataListClie.DataBind();
                }
            }
            ...
            {   //Solo Articulos Relacionados
                ...
                if (radTxtBoxBusqueda.Text == "")
                {
                    ...
                    dsRelAgrAr = (DataSet)Session["dsRelAgrClieSession"];
                    ...
                    DataListClie.DataSource = dsAlmArtClone;
                    DataListClie.DataBind();
                }
            }
            ...
            foreach (DataRow drConfCpto in dsAlmArt.Tables[0].Rows)
            {
                if (Convert.ToString(drConfCpto["valCheck"]) == "1")
                {
                    foreach (DataListItem dlConf in DataListClie.Items)
                    {
                        var objRadCheckBox = dlConf.FindControl("CheckBoxArt") as RadCheckBox;

                        if (objRadCheckBox.Value == Convert.ToString(drConfCpto["cliCve"]))
                        {
    ...
    protected void rCboAgrupacionesDatos_SelectedIndexChanged(object sender, RadComboBoxSelectedIndexChangedEventArgs e)
    {
        ...
        dsRelAgrAr = CargaDataList(57);
        Session["dsRelAgrClieSession"] = dsRelAgrAr;
        ...
        if (CheckBoxTodos.Checked == true)
        {
            foreach (DataListItem dlConf in DataListClie.Items)
            {
                ...
                foreach (DataRow drConfCpto in dsAlmArt.Tables[0].Rows)
                {
                    if (CheckBoxTodos.Checked == true)
                    {
                        if (drConfCpto["cliCve"].ToString() == objRadCheckBox.Value.ToString())
                        {
    ...
    private void LlenaCboDatosAgrupacion()
    {
        ...
        ProcBD.AgregarParametrosProcedimiento("@agrTipId", DbType.Int64, 0, ParameterDirection.Input, 2);
    ...
    private void LlenaComboAgrupaciones()
    {
        ...
        ProcBD.AgregarParametrosProcedimiento("@agrTipId", DbType.Int32, 0, ParameterDirection.Input, 2);
        ProcBD.AgregarParametrosProcedimiento("@ciaCve", DbType.String, 10, ParameterDirection.Input, Pag_sCompania);
        ds = oWS.ObtenerDatasetDesdeProcedimiento(ProcBD.ObtenerXmlProcedimiento(), Pag_sConexionLog);
    ...
    private DataSet CargaDataList(int opc)
    {
        ...
        if (radTxtBoxBusqueda.Text != "")
        {
            ProcBD.AgregarParametrosProcedimiento("@BusquedaCve", DbType.String, 40, ParameterDirection.Input, radTxtBoxBusqueda.Text.Trim());
        }
        else
        {
            ProcBD.AgregarParametrosProcedimiento("@BusquedaCve", DbType.String, 40, ParameterDirection.Input, "");
        }
        ds = oWS.ObtenerDatasetDesdeProcedimiento(ProcBD.ObtenerXmlProcedimiento(), Pag_sConexionLog);
        if (FnValAdoNet.bDSRowsIsFill(ds))
        {
            DataListClie.DataSource = ds;
            DataListClie.DataBind();
            CheckVal();
            CheckAll();
            return ds;
    ...
    private void CheckAll()
    {
        ...
        foreach (DataListItem dlConf in DataListClie.Items)
        {
            Session["IDartCve"] = DataListClie.DataKeys[Countkeyarray].ToString();
            Countkeyarray += 1;
        }
    ...
    protected void CheckBoxArt_CheckedChanged(object sender, EventArgs e)
    {
        ...
        DataRow[] drAlmArt;
        drAlmArt = dsAlmArt.Tables[0].Select("cliCve = '" + rcheckStr + "' ");
        ...
        foreach (DataRow drConfCpto in dsAlmArt.Tables[0].Rows)
        {
            ...
            if (Convert.ToString(drConfCpto["valCheck"]) == "1")
            {
                try
                {
                    ...
                    ProcBD.AgregarParametrosProcedimiento("@agrCve", DbType.String, 10, ParameterDirection.Input, this.rCboAgrupaciones.SelectedValue.ToString());
                    ProcBD.AgregarParametrosProcedimiento("@agrDatoCve", DbType.String, 20, ParameterDirection.Input, this.rCboAgrupacionesDatos.SelectedValue.Trim().ToString());
                    ProcBD.AgregarParametrosProcedimiento("@cveFolio", DbType.String, 30, ParameterDirection.Input, Convert.ToString(drConfCpto["cliCve"]));
                    ProcBD.AgregarParametrosProcedimiento("@agrTipId", DbType.Int32, 0, ParameterDirection.Input, 2);
